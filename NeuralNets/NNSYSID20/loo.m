...
%  [Eloo,H] = loo(NetDef,W1,W2,PHI,Y,trparms) with trparms.maxiter>0, the network
...
%
%  SEE the function "fpe" for the final prediction error estimate
%
%  REFERENCE:
...
if trparms.maxiter>0,
  ...
  for i=1:N,
    ...
    [W1x,W2x,PI_vector,iteration,lambda]=...
                 marq(NetDef,W1,W2,PHI(:,notbeta),Y(:,notbeta),trparms);
...
Eloo=0;
eloovec=zeros(N,1); % Slet igen
for beta=1:N,
  hjh=(PSI_red(:,beta)'*H_inv*PSI_red(:,beta));
  Eloo=Eloo+E(beta)*E(beta)*(1+hjh)/(1-hjh);
  eloovec(beta)=E(beta)*E(beta)*(1+hjh)/(1-hjh); % Slet igen
end
Eloo=Eloo/(2*N);
end