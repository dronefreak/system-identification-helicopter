...
for k=1:1, %dummy loop
  ...
  set(guihand,'Name','NNARX demonstration');

  % >>>>>>>>>>>>>>>>  SCREEN 1  <<<<<<<<<<<<<<<<<
  s0='1';
  s1='The purpose of this demo is to show how the NNARX';
  ...
  s3=[];
  s4='Let''s run the NNARX function, which uses a Levenberg-';
  s5='Marquardt algorithm for generating a NNARX-model.';
  smat=str2mat(s0,s1,s2,s3,s4,s5);
  
  % ----- Choose training parameters -----
  trparms = settrain;
  trparms = settrain(trparms,'maxiter',100,'D',1e-4);
  NetDef = ['HHHHH'
            'L----']; 
  NN = [2 2 1];
  pmnshow(smat,guihand,edmulti,contbut,quitbut);
  ...
  drawnow
  [W1,W2,NSSEvec,iteration,lambda]=nnarx(NetDef,NN,[],[],trparms,y1,u1);
  delete(gca);
